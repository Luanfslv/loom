 ...
 */
function shouldSample(sessionId, sampleRate) {
    if (sampleRate >= 1.0)
        return true;
    if (sampleRate <= 0.0)
        return false;
...
export class SessionTracker {
    ...
    endTimeout;
    // Visibility tracking
    visibilityTimeoutMs = 30 * 60 * 1000; // 30 minutes
    ...
    async start() {
        ...
        try {
            const response = await this.httpClient.postJson(this.getStartEndpoint(), {
