...
import { SseConnection, DEFAULT_SSE_CONFIG } from './sse';
import { InvalidSdkKeyError, InvalidBaseUrlError, AuthenticationError, RateLimitedError, InitializationTimeoutError, ClientClosedError, FlagNotFoundError, ServerError, OfflineNoCacheError, ConnectionError } from './errors';
...
export class FlagsClient {
    ...
    async initialize() {
        ...
        try {
            const response = await this.httpClient.post('/api/flags/evaluate', { context: { environment: '' } }, { signal: controller.signal, retry: false });
            ...
            if (this.enableStreaming) {
                this.sseConnection.start(`${this.baseUrl}/api/flags/stream`, this.sdkKey, this.cache, this.sseConfig);
            }
        }
        catch (e) {
     ...
     */
    async getBool(flagKey, context, defaultValue) {
        this.checkClosed();
     ...
     */
    async getString(flagKey, context, defaultValue) {
        this.checkClosed();
        const result = await this.evaluateFlag(flagKey, context);
     ...
     */
    async getJson(flagKey, context, defaultValue) {
        this.checkClosed();
        const result = await this.evaluateFlag(flagKey, context);
    ...
    async evaluateFlagServer(flagKey, context) {
    ...
    evaluateFlagCached(flagKey) {
        ...
        const reason = flag.archived || !flag.enabled ? { type: 'Disabled' } : { type: 'Default' };
        return {
            flagKey,
            variant: flag.defaultVariant,
            value: flag.defaultValue,
    ...
    async evaluateAllServer(context) {
    ...
    evaluateAllCached() {
        ...
        const results = flags
            ...
            reason: f.enabled ? { type: 'Default' } : { type: 'Disabled' }
        }));
        return {
            results,
