...
export class CronsClient {
     ...
     */
    async checkinStart(monitorSlug) {
        if (this.isClosed) {
        ...
        const response = await this.httpClient.postJson(`/api/crons/monitors/${encodeURIComponent(monitorSlug)}/checkins`, request);
        if (this.debug) {
     ...
     */
    async checkinOk(checkinId, details) {
        if (this.isClosed) {
            throw new ClientClosedError();
        }
        const request = {
        ...
        await this.httpClient.patch(`/api/crons/checkins/${encodeURIComponent(checkinId)}`, request);
        if (this.debug) {
     ...
     */
    async checkinError(checkinId, details) {
        if (this.isClosed) {
            throw new ClientClosedError();
        }
        const request = {
     ...
     * ```
     */
    async withMonitor(monitorSlug, fn) {
        if (this.isClosed) {
            throw new ClientClosedError();
        }
